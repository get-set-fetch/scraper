...
export default function crudResource(storage: Storage) {
  describe(`CRUD Resource using ${storage.config.client}`, () => {
    ...
    it(`${storage.config.client} resource getPagedResources - where(projectId), offset, limit`, async () => {
      await Resource.delAll();
      for (let i = 1; i < 4; i += 1) {
        const resource = new Resource({ projectId: 1, url: `urlA${i}`, content: [ [ `title${i}` ] ] });
        // eslint-disable-next-line no-await-in-loop
        await resource.save();
      }

      const page1Resources = await Resource.getPagedResources({ where: { projectId: 1 }, offset: 0, limit: 2 });
      ...
      const page2Resources = await Resource.getPagedResources({ where: { projectId: 1 }, offset: 2, limit: 2 });
    ...
    it(`${storage.config.client} resource getPagedResources - cols, where(projectId), offset, limit`, async () => {
      await Resource.delAll();
      for (let i = 1; i < 4; i += 1) {
        const resource = new Resource({ projectId: 1, url: `urlA${i}`, content: [ [ `title${i}` ] ] });
        // eslint-disable-next-line no-await-in-loop
      ...
      const page1Resources = await Resource.getPagedResources({ where: { projectId: 1 }, offset: 0, limit: 2, cols: [ 'url' ] });
      const page1Urls = page1Resources.map(resource => resource.url);
      const page1Content = page1Resources.map(resource => resource.content);
      assert.sameMembers(page1Urls, [ 'urlA1', 'urlA2' ]);
      assert.sameDeepMembers(page1Content, [ undefined, undefined ]);

      const page2Resources = await Resource.getPagedResources({ where: { projectId: 1 }, offset: 2, limit: 2, cols: [ 'url' ] });
      const page2Urls = page2Resources.map(resource => resource.url);
      const page2Content = page2Resources.map(resource => resource.content);
      assert.sameMembers(page2Urls, [ 'urlA3' ]);
    ...
    it(`${storage.config.client} resource getPagedResources - cols, where(projectId), whereNotNull(data, content)`, async () => {
      await Resource.delAll();
      for (let i = 1; i < 5; i += 1) {
        const resource = new Resource({ projectId: 1, url: `urlA${i}` });
        if (i % 2 === 0) {
      ...
      const textResources = await Resource.getPagedResources({ where: { projectId: 1 }, whereNotNull: [ 'content' ], cols: [ 'url', 'content' ] });
      ...
      const binaryResources = await Resource.getPagedResources({ where: { projectId: 1 }, whereNotNull: [ 'data' ], cols: [ 'url', 'data' ] });
